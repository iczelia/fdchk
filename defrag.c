...
/*  Pass 1: depth-first, assign new positions - root files first
    (alphabetical), then each subdir (alphabetical, recursing in).  */
...
/*  The transactional defragmenter.  */
static int defrag_worker(DiskHandle * dh, int * moved_out, int * zeroed_out) {
  ...
  const WORD chunk   = (WORD) (G.spt > 0 ? G.spt : 18);   /*  one track  */
  ...
  /*  Bad clusters and the reserved FAT[0]/FAT[1] are pinned.  */
  fixed[0] = fixed[1] = 1;
